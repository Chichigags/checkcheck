...
import { isWeekendDate } from './weekday'
...
function relationPack(kind: DailyRelationKind, zh: boolean, weekend: boolean): CopyPack {
  if (kind === 'wealth') {
    if (weekend) {
      return zh
        ? {
            ...
            paragraphs: [
              '出门或滑手机时，比较容易被“好像很值”打动。家里能收尾的小事可以做：把该回的消息回掉、把一笔账对清楚。新开一个很大的花钱计划，先放一放。',
        ...
        : {
            ...
            paragraphs: [
              'It’s easy to get pulled toward something that looks like a deal. Finish a small thing at home — the message you’ve been putting off, or a number that needs confirming. Leave a big new spend for another day.',
    ...
    if (weekend) {
      return zh
        ? {
            ...
            paragraphs: [
              '跟亲近的人把卡住的话说明白，会比闷着更轻松。说完就停，别把整个下午都用来解释。',
              '别人一句普通的回应，也可能让你反复琢磨。重要的事情别靠猜，直接问清楚。',
            ],
          }
        : {
            headline: 'Say it clearly, then stop',
            paragraphs: [
              'A fuzzy feeling gets lighter if you name it to someone close. Say it once, then stop — don’t spend the afternoon explaining.',
              'A short reply can sit in your head longer than it should. If it matters, ask directly.',
            ],
          }
    }
    return zh
    ...
    if (weekend) {
      return zh
        ? {
            ...
            paragraphs: [
              '把睡眠、吃饭和节奏补回来，会比硬排满一天更有用。今天不是靠意志力硬扛的日子。',
              '如果脑子转得慢，允许自己把事情拆小。先做能看见结果的一小步，再决定要不要出门或加码。',
            ],
          }
        : {
            headline: 'Fill the tank, don’t force it',
            paragraphs: [
              'Sleep, food, and a slower pace will return more than packing the day. White-knuckling it will cost more than it returns.',
              'If your mind feels slower, shrink the plan. Finish one small visible step before you add more.',
            ],
          }
    }
    return zh
  ...
  if (kind === 'authority') {
    if (weekend) {
      return zh
        ? {
            headline: '今天先别硬刚，把对错放一放',
            paragraphs: [
              '家里或跟熟人相处，把话说清楚会比争“谁对”更省力。规则可以明天再对齐。',
              '火气上来时先别回那条重要消息。降温后再说，语气会稳很多。',
            ],
          }
        : {
            headline: 'Skip the fight, keep the peace',
            paragraphs: [
              'At home or with people you know, naming what you need beats arguing who’s right. Let the rules wait until tomorrow.',
  ...
  if (weekend) {
    return zh
      ? {
          headline: '今天适合短聚，别陷入比较',
          paragraphs: [
            '跟人一起做一件小事可以，长时间较劲或反复对照别人的周末，今天特别耗神。走自己的步速就好。',
            '如果有人让你觉得自己慢了，先回到你已经想做的那件小事。把那件做完，比赢过旁边的人更有用。',
          ],
        }
      : {
          headline: 'Keep company short, skip the comparison',
          paragraphs: [
            'A short hangout can help today. A long stretch of matching someone else’s weekend, or measuring yourself against them, will drain you.',
            'If you feel behind, return to the small thing you already wanted to do. Finishing that beats winning the comparison.',
          ],
        }
  }
...
function frictionParagraph(zh: boolean, labels: string[], weekend: boolean): string | null {
  ...
  if (label === 'Month') {
    if (weekend) {
      return zh
        ? '近阶段的生活节奏今天容易卡住。已经开始的小事继续推可以，临时加一个很大的新目标，容易越想越复杂。'
        : 'This season’s pace may snag more easily. Keep moving a small thing you already started; a brand-new oversized goal will get tangled.'
    }
    return zh
  ...
  if (label === 'Hour') {
    return zh
      ? '晚上的精力可能不太稳。重要的事尽量放在白天处理，傍晚之后少做需要判断力的决定。'
      : 'Evening energy may be less steady. Handle anything important earlier in the day if you can, and save judgment calls for later.'
  }
  return zh
...
function harmonyParagraph(zh: boolean, labels: string[], weekend: boolean): string | null {
  const label = labels[0]
  if (!label) return null
  if (label === 'Month') {
    if (weekend) {
      return zh
        ? '近阶段在调的生活节奏，今天可以往前挪一小步。不必开新线，把已经开始的那件推进到能看见结果就够。'
        : 'Something already in motion in your days can move a little today. Don’t open a new thread — just get the current one to a visible result.'
    }
    return zh
...
function steadyParagraph(zh: boolean, animal: string, weekend: boolean): string {
  if (zh) {
    if (animal === 'Snake' || animal === 'Ox') {
      return weekend
        ? '今天没有特别难的地方，整体比较稳。更适合把家里或自己的小事收尾，不太适合临时开一个新坑。'
        : '今天没有特别难的地方，整体比较稳。更适合把已经开始的事情做完，不太适合临时开一个新坑。'
    }
    if (animal === 'Horse' || animal === 'Tiger') {
      return weekend
        ? '今天没有特别大的阻力，但人会比较坐不住。把想做的事拆小，做完一件再动下一件。'
        : '今天没有特别大的阻力，但人会比较坐不住。把要做的事拆小，做完一件再动下一件。'
    }
    return '今天没有特别难的地方，整体比较稳。选一件拖着的小事处理掉就很好。'
  }
  if (animal === 'Snake' || animal === 'Ox') {
    return weekend
      ? 'Nothing especially difficult today — overall steady. Better for finishing a small thing at home than opening something new.'
      : 'Nothing especially difficult today — overall steady. Better for finishing what you started than opening a new hole.'
  }
  if (animal === 'Horse' || animal === 'Tiger') {
    return weekend
      ? 'Nothing major is blocking you, but you may feel restless. Split the day small, and finish one piece before starting the next.'
      : 'Nothing major is blocking you, but you may feel restless. Split the work small, and finish one piece before starting the next.'
  }
  return 'Nothing especially difficult today — overall steady. Clearing one small leftover task is enough.'
}

function buildCopy(signals: DailyBaziSignals, zh: boolean, weekend: boolean): CopyPack {
  const base = relationPack(signals.relationKind, zh, weekend)
  const friction = [...signals.punishmentLabels, ...signals.harmLabels, ...signals.clashLabels]
  const overlay =
    frictionParagraph(zh, friction, weekend) ||
    harmonyParagraph(zh, signals.harmonyLabels, weekend) ||
    (signals.isNeutral ? steadyParagraph(zh, signals.todayAnimal, weekend) : null)
...
export function generateMockMessage(profile: UserProfile, customDate?: string): DailyMessage {
  ...
  const copy = buildCopy(signals, zh, isWeekendDate(dateToUse))
